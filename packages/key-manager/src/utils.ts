...
export const nameToPath = async (name: string, hardened: boolean = true): Promise<string> => {
  ...
  for (let i = 0; i < hash.digest.length / chunkSize; i++) {
    numbers.push(
      Buffer.from(
        hash.bytes.slice(i * chunkSize, (i + 1) * chunkSize)
      ).readUInt16LE() + (hardened ? 0x80000000 : 0)
    )
  }
...
export const keyToPeerId = async (key: BIP32Interface): Promise<PeerId> => {
  ...
  return peerIdFromKeys(marshaledPublicKey, marshaledPrivateKey)
}
...
export const generateKeyFile = async (path: string, mnemonic: string, name: string): Promise<void> => {
