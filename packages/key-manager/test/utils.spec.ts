...
describe('nameToPath', () => {
  ...
  before(async () => {
    hardenedPaths = await Promise.all(data.map(async d => nameToPath(d.name)))
    unhardenedPaths = await Promise.all(data.map(async d => nameToPath(d.name, false)))
    allPaths = [...hardenedPaths, ...unhardenedPaths]
  })

  it('is determnistic', async () => {
    const paths1 = await Promise.all(data.map(async d => nameToPath(d.name)))
    const paths2 = await Promise.all(data.map(async d => nameToPath(d.name)))
...
describe('encodeKey', () => {
  ...
  it('throws an error if the private data is missing', async () => {
    for (const { publicKey: k } of data) {
      ...
      await assert.rejects(async () => encodeKey(key))
    }
  })
})
...
describe('keyToPeerId', () => {
  it('converts keys to peerIds', async () => {
    await Promise.all(data.map(async d => keyToPeerId(decodeKey(d.key))))
  })
})
