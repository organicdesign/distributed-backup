...
type Config = Pick<ImporterConfig, 'chunker' | 'hasher' | 'cidVersion'>

export const importFile = async (blockstore: Blockstore, path: string, config: Config): Promise<ImportResult> => {
...
export const importDir = async function * (blockstore: Blockstore, path: string, config: Config): AsyncIterable<ImportResult & { path: string }> {
