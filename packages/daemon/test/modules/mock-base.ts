...
const provs = (path?: string): Provides => {
  return {
    ...
    datastore: path == null ? new MemoryDatastore() : new FsDatastore(Path.join(path, 'datastore')),
    blockstore: path == null ? new MemoryBlockstore() : new FsBlockstore(Path.join(path, 'blockstore'))
  }
}
